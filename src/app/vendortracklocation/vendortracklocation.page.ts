...
@Component({
  selector: 'app-vendortracklocation',
  templateUrl: './vendortracklocation.page.html',
  styleUrls: ['./vendortracklocation.page.scss'],
})
export class VendortracklocationPage implements OnInit   {
  errorMessage: any;
  latitude: any;
  longitude: any;
  zoom: number;
...
ngOnInit() {
  ...
  TrackLocation(interval) {
    ...
    this.track.VendorLongitude=this.longitude;
     this.vendorTrackLocationService.GetVendorTrack(this.track).subscribe(
       (data) => {
           if (data) {
